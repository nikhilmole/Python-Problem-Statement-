...
def MailSender(filename,time,Resmail):
    try:
...
def ProcessLog(email,log_dir = 'Demo'):
    listprocess = []
    ...
    if connected:
        starttime = time.time()
        MailSender(log_path,time.ctime(),email)
        endtime = time.time()
...
def main():
    ...
    try:
        ...
        email = sys.argv[2]
        schedule.every(interval).minutes.do(ProcessLog, email=email)
        while True:
